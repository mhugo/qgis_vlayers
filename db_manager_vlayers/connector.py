...
def get_query_geometry_name( sqlite_file ):
    ...
    c = conn.cursor()
    for r in c.execute( "SELECT name, type FROM _columns WHERE table_id=0" ):
        name, type = r
        if ':' in type and type != 'no::':
            return name
            break
    return None
                ...
                for _, l in QgsMapLayerRegistry.instance().mapLayers().items():
                    if l.type() == QgsMapLayer.VectorLayer:
                        ...
                        if g == QGis.WKBPoint:
                            ...
                            geomType = 'MULTIPOLYGON'
                            dim = 'XYZ'
                        lst.append( (Table.VectorType, lname, False, False, l.id(), 'geometry', geomType, dim, l.crs().postgisSrid()) )
                return lst
